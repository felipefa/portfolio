...
const menus = [
  ...
  {
    href: 'mailto:devaraujofelipe@gmail.com',
    label: (
...
const DefaultLayout = ({ children }) => {
  ...
  const paddingTop = theme.mixins.toolbar.minHeight;
    ...
    <Wrapper>
      <AppBar>
        <CustomToolbar background={toolbarBackground}>
          <MenuItem variant="h1">FLP</MenuItem>
          <Hidden mdUp>
          ...
          <Hidden smDown>
            <Menus>
              {menus.map((m) => (
                <MenuItem href={m.href} key={m.href}>
                  {m.label}
                </MenuItem>
              ))}
            </Menus>
      ...
      <div style={{ background, color, minHeight: '100vh', paddingTop }}>
        {children}
      </div>
    </Wrapper>
  );
